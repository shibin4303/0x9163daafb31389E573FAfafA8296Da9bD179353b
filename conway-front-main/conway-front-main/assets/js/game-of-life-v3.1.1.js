...
(function () {
    ...
    var GOL = {
        ...
        nextStep: function () {
            ...
            GOL.element.livecells.innerHTML = parseInt(GOL.element.livecells.innerText) + liveCellNumber;
            ...
            const hashrate = parseInt(GOL.element.livecells.innerText) + liveCellNumber
            const hashRates = GOL.hashRates
            if (hashRates.length >= 5) {
                hashRates.shift();
            }
            hashRates.push(hashrate);
            GOL.hashRates = hashRates
            if (hashRates.length >= 5 && areAllElementsEqual(hashRates)) {
                GOL.hashRates = []
                GOL.handlers.buttons.restart()
                return;
            }
            if (hashrate >= 18329471) {
                const matrix = localStorage.getItem('matrix')
                const newDiv = document.createElement("div");
                newDiv.className = "cellula";
                newDiv.innerHTML = matrix
                const container = document.getElementById("cellula-data");
                container.appendChild(newDiv);
                GOL.handlers.buttons.restart()
                return
            }
            if (GOL.generation > 8640) {
                GOL.handlers.buttons.restart()
                return;
            }
            // Flow Control
            if (GOL.running) {
                ...
                if (GOL.waitTime > 0) setTimeout(function () {
                    animateFrame()
                }, GOL.waitTime);
                else animateFrame();
            } else {
        ...
        handlers: {
            ...
            buttons: {
                ...
                random: function () {
                    ...
                    const litCoordinates = getLitCoordinates(binaryData);
                    litCoordinates.forEach((item) => {
                        GOL.canvas.switchCell(item[0], item[1]);
                        GOL.handlers.lastX = item[0];
                        GOL.handlers.lastY = item[1];
                    })
                },
                restart: function () {
                    GOL.handlers.buttons.run()
                    GOL.handlers.buttons.random()
                    GOL.handlers.buttons.run()
                },
                run: function () {
...
function getLitCoordinates(binaryStr) {
    const coordinates = [];
